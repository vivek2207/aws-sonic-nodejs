...
# AWS Sonic Node.js Server

This is a Node.js server implementation for AWS Sonic - a banking assistant that uses AWS Bedrock for AI capabilities combined with a knowledge base for accurate banking information.

## Recent Changes
- Updated system prompts to ensure only information from the knowledge base is displayed in responses
- Removed all hardcoded default user information to prevent displaying values not from the knowledge base
- Modified user information handling to only include verified customer data from DynamoDB
- Enhanced response validation to prioritize knowledge base responses over model-generated content
- Fixed issues with response handling to ensure only accurate information is returned to users

## Setup

1. Ensure you have Node.js installed (v14.x or higher)
2. Clone this repository
3. Install dependencies:
   ```
   npm install
   ```
4. Configure AWS credentials (ensure you have access to AWS Bedrock)
5. Run the server:
   ```
   npm run dev
   ```
...
- Semantic search for banking information using AWS Bedrock knowledge bases
- Voice-based conversational interface
- User verification via phone number
- Personalized responses based on user information
- Strict information validation to ensure accuracy

## Configuration

- AWS credentials should be set up in your environment or profile
- The knowledge base ID should be configured in `src/server.ts`
