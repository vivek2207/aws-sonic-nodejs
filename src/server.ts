    ...
    try {
        ...
        session.onEvent('textOutput', async (data) => {
            ...
            if (data.role === 'USER') {
                ...
                // For assistant responses, ensure they are concise
                const content = data.content || '';
                const conciseContent = content.split('.')[0] + '.';
                socket.emit('textOutput', {
                    ...data,
        ...
        socket.on('systemPrompt', async (data) => {
            try {
                ...
                // Use our banking-specific system prompt
                await session.setupSystemPrompt(undefined, getSystemPrompt());
            } catch (error) {
