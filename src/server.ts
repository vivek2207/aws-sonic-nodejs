...
import { getUserByPhoneNumber, getRandomUser, formatUserData } from './dynamodb-service';
...
// Login API endpoints - Fix TypeScript types
app.post('/api/login', (req: Request, res: Response) => {
    // Use non-async function definition
    getUserByPhoneNumber(req.body.phoneNumber)
        .then(userData => {
            ...
            if (!userData) {
                return res.status(404).json({ message: 'User not found', userData: null });
            }
...
app.post('/api/login/random', (req: Request, res: Response) => {
    // Use non-async function definition
    getRandomUser()
        .then(userData => {
            if (!userData) {
                return res.status(500).json({ message: 'Failed to get random user' });
            }
            
            return res.status(200).json({ message: 'Random login successful', userData });
        })
        .catch(error => {
...
io.on('connection', (socket) => {
    ...
    try {
        ...
        // Handle userData to include in system prompt
            ...
            try {
                ...
                socket.on('userData', async (userData) => {
                    try {
                        ...
                        // Format user data for system prompt
                        const formattedUserData = formatUserData(userData);
                        
                        // Create a new system prompt with user data
                        const systemPromptWithUserData = `${data}\n\nUser Info: ${formattedUserData}`;
                        
                        // Set the system prompt with user data
                        await session.setupSystemPrompt(undefined, systemPromptWithUserData);
                    } catch (error) {
                ...
                // If no userData event is received within 1 second, use default system prompt
                const userDataTimeout = setTimeout(async () => {
                    if (!socket.listeners('userData').length) {
                        await session.setupSystemPrompt(undefined, data);
                    }
                }, 1000);
                
                // Clear timeout when userData is received
                socket.on('userData', () => clearTimeout(userDataTimeout));
